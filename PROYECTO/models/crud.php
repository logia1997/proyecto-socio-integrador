...
//funciones añadidas por luis
		...
		public static function listaMesesAguaSinPagar($idCasa){
			...
			$stmt = Conexion::conectar()->prepare("SELECT * FROM agua WHERE (idCasa = $idCasa) AND (idEstado = 4 )");
			$stmt -> execute();
		...
		public static function ingresarPagoAgua($idAgua, $idPago){
			    $consulta ="
				UPDATE `agua` SET `idEstado` = '6',`idPago` = '$idPago' WHERE id = $idAgua";
				$stmt = Conexion::conectar()->prepare($consulta);	
				$stmt->execute();
		}
		...
		public static function  retornarNoticiaActual($dataName, $departamento){
			$stmt = Conexion::conectar()->prepare("SELECT * FROM $dataName WHERE (departamento = $departamento) AND (prioridad = 1)");
			$stmt -> execute();
			$lista= $stmt -> fetchall();
			return $lista[0] ;
		}

		public static function  retornarNoticiaAnterior($dataName, $departamento, $id){
			$anterior;

			$stmt = Conexion::conectar()->prepare("SELECT * FROM $dataName WHERE (departamento = $departamento) AND (prioridad = 1)");
			$stmt -> execute();
			$lista2= $stmt -> fetchall();
			$idReciente=$lista2[0][0];

	



			$stmt = Conexion::conectar()->prepare("SELECT * FROM $dataName WHERE (departamento = $departamento) AND (prioridad = 2)");
			$stmt -> execute();
			$lista= $stmt -> fetchall();
			...
			if($lista[0][0] == $id) return $lista[0];

			$anterior= $lista[0];

			foreach ($lista as $registro){
				if($registro[0]==$id) return $anterior;
				else $anterior=$registro;
